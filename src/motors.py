...
class TurretHardware:
    ...
    def step_synchronized(self, motor_commands):
        \"\"\"
        Executes steps for multiple motors simultaneously to reduce jitter.
        motor_commands: List of tuples (motor, direction, delay)
        \"\"\"
        # 1. Set all directions
        for motor, direction, delay in motor_commands:
            GPIO.output(motor.dir_pin, GPIO.HIGH if direction == 1 else GPIO.LOW)
            
        # 2. Set all step pins HIGH
        for motor, direction, delay in motor_commands:
            GPIO.output(motor.step_pin, GPIO.HIGH)
            
        # 3. Pulse width: use the minimum delay to ensure stability
        min_delay = min(cmd[2] for cmd in motor_commands)
        time.sleep(min_delay)
        
        # 4. Set all step pins LOW
        for motor, direction, delay in motor_commands:
